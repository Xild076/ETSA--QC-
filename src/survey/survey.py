...
def display_consent_form():
    ...
    st.markdown("""Thank you for your interest in our research study. Please read the following information carefully before deciding to participate. **Purpose of the Study:** This research aims to understand how people perceive sentiment in complex sentences. We are exploring how sentence structure and word choices influence the sentiment attributed to various **highlighted entities** (persons or groups) within those sentences. Your responses will contribute to the development of more nuanced natural language understanding models. **Your Participation:** - Participation is entirely voluntary. You must be 18 years or older. - The survey involves reading 20 unique, synthetically generated sentences. For each sentence, you will rate the sentiment you feel is directed towards specific highlighted entities. - Estimated time: 10-15 minutes. - You can withdraw at any time by closing your browser, without penalty. Data collected up to that point will not be used. **Data Confidentiality:** - Responses are anonymized. We do not collect personally identifiable information like your name or IP address. - Anonymized data will be used for academic research, including potential publications and presentations. It may also be shared with other researchers or made publicly available to benefit the scientific community. **Risks and Benefits:** - No known significant risks. Some sentences may describe negative scenarios as part of the research design. - No direct personal benefit, but your contribution is valuable to advancing linguistic research. **Contact Information:** For questions, please contact [Your Name/Research Group Name] at [Your Email Address or Contact Link]. By clicking 'I Agree and Start Survey', you confirm you have read and understood this information, are 18 or older, and consent to participate under these conditions.""")
    if st.button("I Agree and Start Survey", type="primary", use_container_width=True):
        st.session_state.consent_given = True; st.session_state.sentences_data = generate_sentences_data()
        if not st.session_state.sentences_data: st.error("Failed to generate sentences. Please try again."); return
        st.session_state.shuffled_indices = list(range(len(st.session_state.sentences_data))); random.shuffle(st.session_state.shuffled_indices)
        st.session_state.current_question_index = 0; st.session_state.survey_complete = False
        st.session_state.user_responses = []; st.session_state.current_scores = {}
        st.rerun()
...
def export_to_google_sheets(data_df):
    ...
    if not creds_str: st.error(f"Secret '{GOOGLE_CREDENTIALS_SECRET_KEY}' not found."); return
    try: creds_dict = json.loads(creds_str)
    except json.JSONDecodeError: st.error("Failed to parse Google credentials JSON."); return
    
    try:
        creds = Credentials.from_service_account_info(creds_dict, scopes=["https://www.googleapis.com/auth/spreadsheets"])
        client = gspread.authorize(creds)
        if GOOGLE_SHEET_ID == "YOUR_SPREADSHEET_ID_HERE": st.error("GOOGLE_SHEET_ID not set."); return
        ...
        st.success(f"Data exported to Google Sheet (ID: {GOOGLE_SHEET_ID})")
    except gspread.exceptions.SpreadsheetNotFound: st.error(f"Sheet (ID: '{GOOGLE_SHEET_ID}') not found or not shared with {creds_dict.get('client_email', 'service account')}.")
    except Exception as e: st.error(f"Google Sheets export error: {e}"); st.info(f"Check secrets, Sheet ID, & sharing with {creds_dict.get('client_email', 'service account')}.")
...
def display_finish_screen():
    ...
    if st.session_state.user_responses:
        ...
        if GOOGLE_SHEET_ID != "YOUR_SPREADSHEET_ID_HERE" and st.secrets.get(GOOGLE_CREDENTIALS_SECRET_KEY):
            if st.button("Export Data to Google Sheets", type="primary", use_container_width=True): export_to_google_sheets(df)
        elif GOOGLE_SHEET_ID == "YOUR_SPREADSHEET_ID_HERE": st.warning("Google Sheet Export not configured by admin (GOOGLE_SHEET_ID).")
        elif not st.secrets.get(GOOGLE_CREDENTIALS_SECRET_KEY): st.warning(f"Google Sheet Export not configured by admin ('{GOOGLE_CREDENTIALS_SECRET_KEY}' secret).")
