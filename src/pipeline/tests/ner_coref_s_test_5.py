...
class SaveManager:
    def __init__(self, appcfg: dict):
        self.appcfg = appcfg
        io = appcfg["io"]
        self.rules_dir = io["rules_dir"]
        self.logs_dir = io["logs_dir"]
        self.params_dir = io["params_dir"]
        os.makedirs(self.rules_dir, exist_ok=True)
        os.makedirs(os.path.join(self.rules_dir, "best"), exist_ok=True)
        os.makedirs(self.logs_dir, exist_ok=True)
        os.makedirs(self.params_dir, exist_ok=True)
        self.compress = io.get("compress", True)
        self.schema = {"version": appcfg["meta"]["version"], "signature_schema": "json_tuple_v1"}
    ...
    def save_payload(self, payload: dict, mark_best: bool = False):
        ts = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
        base = f"rules_{ts}_{uuid.uuid4().hex[:8]}"
        if self.compress:
            path = os.path.join(self.rules_dir, base + ".json.gz")
            raw = json.dumps(payload, ensure_ascii=False).encode("utf8")
            buf = gzip.compress(raw, compresslevel=5)
            h = self._hash_bytes(buf)
            self._atomic_write(path, buf)
        else:
            path = os.path.join(self.rules_dir, base + ".json")
            raw = json.dumps(payload, ensure_ascii=False).encode("utf8")
            h = self._hash_bytes(raw)
            self._atomic_write(path, raw)
        manifest = {"path": path, "hash": h, "schema": self.schema, "saved_at": dt.datetime.now().isoformat()}
        if mark_best:
            best_id = f"best_{ts}_{h}.json"
            best_path = os.path.join(self.rules_dir, "best", best_id)
            if self.compress:
                self._atomic_write(best_path + ".gz", buf)
                latest = {"file": best_id + ".gz", "hash": h, "time": manifest["saved_at"]}
            else:
                self._atomic_write(best_path, raw)
                latest = {"file": best_id, "hash": h, "time": manifest["saved_at"]}
            with open(os.path.join(self.rules_dir, "best", "LATEST.json"), "w", encoding="utf8") as f:
                json.dump(latest, f)
        return path, manifest

    def load_latest_best(self):
        latest_txt = os.path.join(self.rules_dir, "best", "LATEST.json")
        if not os.path.exists(latest_txt):
            bests = sorted(glob.glob(os.path.join(self.rules_dir, "best", "best_*.json*")))
            if not bests:
                return None
            path = bests[-1]
            return self._load_file(path)
        with open(latest_txt, "r", encoding="utf8") as f:
            meta = json.load(f)
        path = os.path.join(self.rules_dir, "best", meta["file"])
        return self._load_file(path)
...
class MultiStageRuleExtractor:
    def __init__(self, cfg: TrainConfig, appcfg: dict | None = None):
        if appcfg is None:
            appcfg = default_app_config()
        self.appcfg = appcfg
        self.saver = SaveManager(appcfg)
        rng_seed = appcfg["runtime"].get("seed", 13)
        ...
        self._rule_cache = {}
        self.precision_guard_memo = defaultdict(int)

    @lru_cache(maxsize=32768)
    def _np_text_cached(self, doc_id, i, left, right):
        return " ".join(t.text for t in sorted(left + [i] + right))
    ...
    def _np_text(self, token):
        ...
        return " ".join(t.text for t in (left + [token] + right))

    def _compound_len(self, token):
        c = 1
        for t in token.lefts:
            if t.dep_ in {"compound","amod","nummod","flat"}:
                c += 1
        for t in token.rights:
            if t.dep_ in {"compound","amod","nummod","flat","appos"}:
                c += 1
        return c
    ...
    def _sig_variants(self, token, doc, chunk_map):
        if self.cfg.soft_restrict:
            ...
            if token.lemma_.lower() not in self.aspect_lex_head_lemmas:
                if not pos_ok:
                    return []
    ...
    def prepare(self, raw_items):
        ...
        texts = [re.sub(r"\s+", " ", it["text"]).strip() for it in raw_items]
        t0 = time.time()
    ...
    def _allowed_by_context(self, tok, gate_union):
        ...
        if hl in gate_union:
            return True
        if hl in self.aspect_lex_head_lemmas:
            return True
        if hl in self.frequent_single_gold_heads:
            return True
        if self._compound_len(tok) >= 2 and hl not in self.generic_block_heads:
            return True
        if self.cfg.use_entities and tok.ent_type_ in {"PRODUCT","ORG","WORK_OF_ART","MONEY","FAC"}:
            return True
        return False

    def _precision_guard(self, tok, gate_union):
        hl = tok.lemma_.lower()
        if hl in self.aspect_lex_head_lemmas:
            return True
        if self._compound_len(tok) >= 2:
            return True
        if self.cfg.use_entities and tok.ent_type_ in {"MONEY","FAC","ORG","PRODUCT"}:
            return True
        k = hl
        self.precision_guard_memo[k] += 1
        return self.precision_guard_memo[k] > 1
    ...
    def predict_doc(self, doc, sigs, rules):
        ...
        for i,(tok, sv) in enumerate(zip(doc, sigs)):
            ...
            if not self._allowed_by_context(tok, gate_union):
                continue
            if not self._precision_guard(tok, gate_union):
                continue
            if self._valid_surface(surf):
                res.add(surf)
        out = list(res)
    ...
    def _select_by_f1_gain_fast(self, sample, combined, scored, head_hits, budget):
        ...
        gains = []
        cache = {}
        chronic = set(self.chronic_heads)
        for sig, pos, tot, prec in tqdm(scored[:topk], desc="Scoring candidates", unit="cand"):
            ...
            chronic_bonus = 1 if any(h in chronic for h in head_hits.get(sig, set())) else 0
            if m_f1 - base_f1 > 0 and p_ok and r_ok:
                gains.append((m_f1 - base_f1, chronic_bonus, sig, pos, tot, prec, atp, afp, m_p, m_r, m_f1))
                cache[sig] = {"dtp": atp, "dfp": afp, "rule_prec": prec, "mp": m_p, "mr": m_r, "mf1": m_f1}
        gains.sort(key=lambda x: (-x[0], -x[1], -x[5], -x[3]))
        out = []
        for g in gains:
            out.append(g[2])
            if len(out) >= budget:
    ...
    def _harvest_block_phrases(self, prepared, rules, top_k=60):
        ...
        for surf, c in fp_counter.most_common(top_k):
            ...
            if len(parts) <= 2:
                head = parts[-1]
                if head in self.generic_block_heads and c >= 2:
                    new_blocks.add(surf)
        return new_blocks
    ...
    def _prune_rules(self, prepared, existing_rule_sets, new_stage_rules):
        ...
        rng = random.Random(self.appcfg["runtime"].get("seed", 7))
    ...
    def _learn_rules_iteratively(self, residual, existing_rule_sets):
        stage_rules = set()
        stage_allowed = {}
        rng = random.Random(self.appcfg["runtime"].get("seed", 13))
    ...
    def train(self, training_data):
        ...
        for stage in tqdm(range(1, self.cfg.max_stages + 1), desc="Stages", unit="stage"):
            ...
            if p >= targets["precision"] and r >= targets["recall"] and f1 >= targets["f1"]:
                break
            residual = []
    ...
    def predict_batch(self, texts, n_process=None, batch_size=None):
        n_process = n_process if n_process is not None else self.cfg.n_process
        batch_size = batch_size if batch_size is not None else self.cfg.batch_size
        docs = list(self.nlp.pipe(texts, n_process=n_process, batch_size=batch_size))
        out = []
        combined = set().union(*self.rule_stages) if self.rule_stages else set()
        for doc in docs:
            chunk_map = {}
            if self.cfg.use_chunks:
                for nc in doc.noun_chunks:
                    for w in nc:
                        chunk_map[w.i] = nc
            sigs = [self._sig_variants(t, doc, chunk_map) for t in doc]
            out.append(self.predict_doc(doc, sigs, combined))
        return out
    ...
    def evaluate(self, raw_items, dump_prefix=None, top_n_errors=15):
        ...
        if dump_prefix:
            os.makedirs(self.saver.logs_dir, exist_ok=True)
            with open(os.path.join(self.saver.logs_dir, f"{dump_prefix}_fp_top.tsv"),"w",encoding="utf8") as f:
                for term, count in fp_counter.most_common(top_n_errors):
                    f.write(f"{term}\t{count}\n")
            with open(os.path.join(self.saver.logs_dir, f"{dump_prefix}_fn_top.tsv"),"w",encoding="utf8") as f:
    ...
    def _dump_debug(self, prepared, rules, prefix="debug"):
        ...
        with open(os.path.join(self.saver.logs_dir, f"{prefix}_misses.tsv"),"w",encoding="utf8") as f:
        ...
        with open(os.path.join(self.saver.logs_dir, f"{prefix}_fps.tsv"),"w",encoding="utf8") as f:
        ...
        def dump_counter(c, path):
            with open(os.path.join(self.saver.logs_dir, path),"w",encoding="utf8") as f:
        ...
        with open(os.path.join(self.saver.logs_dir, f"{prefix}_miss_summary.json"),"w",encoding="utf8") as f:
            json.dump({
    ...
    def save_rules(self, meta=None, mark_best=False):
        ...
        payload = {
            "schema": self.saver.schema,
            "config": asdict(self.cfg),
            "appcfg": self.appcfg,
            "rule_stages": [[list(x) for x in stage] for stage in rules_flat],
            "rule_allowed_heads": {encode_sig(k): list(v) for k,v in self.rule_allowed_heads.items()},
            "block_phrases": list(self.block_phrases),
            "meta": meta or {}
        }
        path, manifest = self.saver.save_payload(payload, mark_best=mark_best)
        return path

    def load_rules(self, path=None, best=False):
        data = None
        if best:
            data = self.saver.load_latest_best()
        elif path and os.path.exists(path):
            data = self.saver._load_file(path)
        if not data:
            return False
        self.cfg = TrainConfig(**data["config"])
        self.appcfg = data.get("appcfg", self.appcfg)
        stages = []
        for stage in data["rule_stages"]:
            stages.append(set(tuple(x) for x in stage))
        self.rule_stages = stages
        self.rule_allowed_heads = {decode_sig(k): set(v) for k,v in data["rule_allowed_heads"].items()}
        self.block_phrases = set(data.get("block_phrases", []))
        return True
            ...
            if root.tag == "sentences":
                for elem in root.findall(".//sentence"):
                    ...
                    if text_el is None or text_el.text is None:
                        continue
                    txt = re.sub(r"\s+", " ", text_el.text).strip()
                    if not txt:
                        continue
                    ats = []
                    for at in elem.findall(".//aspectTerm"):
                        term = at.get("term"); fr = at.get("from"); to = at.get("to")
                        if term is None or fr is None or to is None:
                            continue
                        ats.append({"term": term, "from": int(fr), "to": int(to)})
                    if ats:
                        all_data.append({"text": txt, "aspects_with_offsets": ats})
            elif root.tag == "Reviews":
                for elem in root.findall(".//sentence"):
                    if elem.get("OutOfScope") == "TRUE":
                        continue
                    text_el = elem.find("text")
                    if text_el is None or text_el.text is None:
                        continue
                    ...
                    if ops:
                        all_data.append({"text": txt, "aspects_with_offsets": ops})
...
def optimize_hyperparams(train_raw, val_raw, base_appcfg: dict, n_trials=25, timeout=None, seed=42):
    def objective(trial: optuna.Trial):
        ...
        extractor = MultiStageRuleExtractor(cfg, appcfg=base_appcfg)
        extractor.train(train_raw)
        p, r, f1 = extractor.evaluate(val_raw, dump_prefix=None)
        meta = {"p": p, "r": r, "f1": f1, "trial": trial.number}
        params_path = os.path.join(extractor.saver.params_dir, "best_params.jsonl")
    ...
    if best_model_path:
        params_dir = base_appcfg["io"]["params_dir"]
        with open(os.path.join(params_dir, "best_overall.json"), "w", encoding="utf8") as f:
...
def default_app_config():
    return {
        "io": {
            "rules_dir": "models/ner_coref/rules",
            "logs_dir": "outputs/ner_coref/logs",
            "params_dir": "outputs/ner_coref/parameters",
            "compress": True
        },
        "runtime": {
            "seed": 13
...
def run_pipeline(train_files, test_files, optimize_trials=0, timeout=None, appcfg: dict | None = None):
    if appcfg is None:
        appcfg = default_app_config()
    ...
    best_cfg = TrainConfig(
        ...
        n_process=1,
        batch_size=4000,
        target_precision=appcfg["targets"]["precision"],
        target_recall=appcfg["targets"]["recall"],
        target_f1=appcfg["targets"]["f1"],
        pr_margin=0.01,
        f1_eval_sample=1200,
        f1_eval_topk=600,
        prune=True,
    ...
    if optimize_trials and optimize_trials > 0:
        study = optimize_hyperparams(train_raw, test_raw, base_appcfg=appcfg, n_trials=optimize_trials, timeout=timeout)
    ...
    extractor = MultiStageRuleExtractor(best_cfg, appcfg=appcfg)
    best_overall_path = os.path.join(appcfg["io"]["params_dir"], "best_overall.json")
    ...
    meta = {"p": p, "r": r, "f1": f1, "timestamp": dt.datetime.now().isoformat(), "appcfg": appcfg}
    path = extractor.save_rules(meta=meta, mark_best=True)
    with open(os.path.join(appcfg["io"]["params_dir"], "best_params.jsonl"), "a", encoding="utf8") as f:
