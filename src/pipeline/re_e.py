...
class GemmaRelationExtractor(RelationExtractor):
    ...
    def _create_prompt(self, sentence: str, entities: List[str]) -> str:
        ...
        return f"""You are a precise relation extraction engine for sentiment analysis. Analyze the sentence to identify specific types of relationships between entities that affect sentiment calculations.
...
1. **ACTION**: Actor performs action on target (for sentiment interdependence)
   - Extract when: One entity performs an action that affects another entity
   - Format: {{"subject": {{"head": "actor_entity"}}, "object": {{"head": "target_entity"}}, "relation": {{"type": "ACTION", "text": "action_phrase"}}}}
   - Example: "The waiter served the food" → actor="waiter", target="food", action="served"
   - Example: "I ordered the pizza" → actor="I", target="pizza", action="ordered"
   - Example: "Chef prepared the meal" → actor="chef", target="meal", action="prepared"

2. **ASSOCIATION**: Entities connected by proximity or explicit connection (for sentiment transfer)
   - Extract when: Entities are linked by conjunctions, groupings, or mentioned together
   - Format: {{"subject": {{"head": "entity1"}}, "object": {{"head": "entity2"}}, "relation": {{"type": "ASSOCIATION", "text": "connection_phrase"}}}}
   - Example: "food and wine" → entity1="food", entity2="wine", connection="and"
   - Example: "pizza with salad" → entity1="pizza", entity2="salad", connection="with"
   - Example: "staff and management" → entity1="staff", entity2="management", connection="and"

3. **BELONGING**: Child entity belongs to/is part of parent entity (for part-whole sentiment)
   - Extract when: One entity is a part, attribute, or possession of another
   - Format: {{"subject": {{"head": "child_entity"}}, "object": {{"head": "parent_entity"}}, "relation": {{"type": "BELONGING", "text": "possession_phrase"}}}}
   - Example: "chef's special dish" → child="dish", parent="chef", possession="'s special"
   - Example: "restaurant's atmosphere" → child="atmosphere", parent="restaurant", possession="'s"
   - Example: "the hotel bar" → child="bar", parent="hotel", possession="the hotel"

**DO NOT EXTRACT**:
- Sentiment modifiers (e.g., "food is good", "service was slow")
- Quality descriptions (e.g., "excellent food", "terrible service")  
- Opinion statements (e.g., "I think", "I feel")
- Simple temporal sequences without entity interaction

**IMPORTANT**: Only extract relations between entities in the provided list. Focus on structural relationships that affect how sentiment propagates between entities.

Output in JSON format:
{{
  "relations": [
    {{"subject": {{"head": "entity1"}}, "object": {{"head": "entity2"}}, "relation": {{"type": "ACTION/ASSOCIATION/BELONGING", "text": "relationship_phrase"}}}},
  ]
}}"""
