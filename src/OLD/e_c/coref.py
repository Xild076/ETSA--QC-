...
def _mav(device: str):
    ...
    try:
        from maverick import Maverick  # type: ignore
    except Exception as exc:
        raise RuntimeError("'maverick' package is required for coreference resolution.") from exc
...
def _sent_spans(text: str) -> List[Tuple[int, int]]:
    """Return list of (start_char, end_char) sentence spans using spaCy."""
    nlp = _spacy()
    return [(s.start_char, s.end_char) for s in nlp(text).sents]

_det = ("the ", "The ", "a ", "A ", "an ", "An ")
_poss = (
    "my ", "My ", "our ", "Our ", "your ", "Your ",
    "his ", "His ", "her ", "Her ", "their ", "Their "
)
_quant = (
    "plenty of ", "Plenty of ", "lots of ", "Lots of ",
    "lot of ", "Lot of ", "bunch of ", "Bunch of ",
    "kind of ", "Kind of ", "sort of ", "Sort of "
)

_stop_entities = {
    "i", "we", "they", "he", "she", "it", "this", "that",
    "these", "those", "here", "there", "rest", "us", "you",
    "someone", "something", "anything", "everything", "nothing",
    "plenty", "favorite", "wannabe", "wannabes"
}
...
def _strip(t: str, s: int) -> Tuple[str, int]:
    """Strip determiners/possessives/quantifiers from the beginning of text t.

    Returns the stripped text and updated start offset.
    """
    for d in _det:
...
def _normalize_to_head(doc: "spacy.tokens.Doc", s: int, e: int, t: str) -> Tuple[str, int, int]:
    """Reduce a span to its head (with left compounds) and return normalized text and offsets."""
...
def resolve(text: str, device: str = "-1") -> Tuple[Dict[int, Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    """Run coreference resolution and basic entity extraction.

    Returns
    -------
    clusters: Mapping of cluster id -> {"entity_references": [(text, [start, end]), ...]}
    sent_map: Mapping of clause key -> {"entities": [(text, [start, end]), ...]}
    """
